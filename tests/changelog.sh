...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/ghost-changelog.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
export GIT_AUTHOR_NAME=ghost-test GIT_AUTHOR_EMAIL=ghost-test@example.com
export GIT_COMMITTER_NAME=ghost-test GIT_COMMITTER_EMAIL=ghost-test@example.com
